...
| `path` | 拼接文件路径 | |
| `package_info_plus` | 读 App 版本号，显示在标题栏 | Flutter 官方插件 |
...
- 水印只能单行。输入框里主动敲回车或粘贴多行文本时，换行会被折叠成空格（不是删除，避免相邻两行首尾粘连）
...
- **平铺满画面**（默认）：文字倾斜重复铺满整张图，无法靠裁剪或局部涂抹去除。网格位置也**可以用手指在预览上拖动**，拖的是整体的偏移
...
- 平铺模式下整层网格也可以拖动，拖的是**网格的整体偏移**（不是某一块），跟手移动
- 拖动改的是**网格的整体偏移**：归一化后 **0 = 铺满，1 = 网格完全移出（100% 空白）**，偏移量夹在 **±3/4（归一化 ±0.75，约对应 72% 空白）**。位移按「网格刚好完全移出画布所需的距离」缩放，所以 offset 与实际露出的空白近似成正比
- 拖出的部分会露出**没有水印的区域**（这是有意为之）：代价是平铺不再能防住「裁掉无水印的那半张」，换来的是用户能自己决定水印往哪边偏移、露出多大区域
- 摆好的偏移量同样会被记住：切换版式再切回来、重启 App 后仍在

拖动时的参考线（两种版式共用）：

- 拖动过程中在预览图上显示十字参考线，松手后隐藏
- 参考线**穿过水印图层的中心**，随水印一起移动，始终标示当前的水印中心
- 单块模式：水印中心是文字实际绘制位置（含贴边时的边界夹取）；平铺模式：水印中心是网格中心，即画布中心加网格的整体偏移
- 参考线只画在照片范围内（超出部分被裁剪）。平铺模式偏移较大时网格中心可能跑到照片外，由裁剪处理，不对位置做夹取

**点击预览区会打开系统相册**：无论空状态（还没选图）还是已选图，点整个预览区都唤起系统相册选图，没有单独的「从相册选照片」按钮。它与「拖动水印」的手势共存，靠 Flutter 手势竞技场自动分流：**轻点 → 打开相册；拖动 → 移动水印（平铺与单块模式都支持）**。
...
页面只有主编辑页一个（选图、输文案、预览、摆位置、保存以及全部样式调整都在这一页），没有独立设置页。最近照片与最近文案都是底部弹层，但入口分处两地：最近照片以「最近照片（N）」出现在标题栏右侧（与「证件水印」标题同行的 AppBar actions，是一个图标 + 文字的按钮），最近文案以「最近文案（N）」出现在水印文字输入框下方、与「插入今天日期」并排（最近文案靠左、插入今天日期靠右）。页面从上到下依次是：标题栏 → 预览区 → 滚动区 → 底部固定条。预览区固定高度 320、**不参与滚动**（里面的拖动手势与页面滚动需要隔离，见第 7.4 节）；文案与样式分两张圆角卡片放在滚动区里（首张是水印文字输入框 + 「最近文案 / 插入今天日期」一行，次张是版式 / 透明度 / 字号 / 颜色）；保存按钮在底部固定条里，不随内容滚动。三块区域（预览、文案、样式）都以圆角卡片形式分组，卡片间间距 16。这是一个工具型 App，不套 fitutor 那样的多 Tab 结构。标题栏在「证件水印」标题后以小字显示 App 版本号（形如 v1.1.0）。
...
`WatermarkDragLayer` 只负责「让用户摆水印」，不负责画水印本身 —— 水印仍由 `photo_canvas` 按 `WatermarkLayout` 的结果绘制。单块模式拖动改 `WatermarkStyle.singlePosition`，平铺模式拖动改网格整体偏移 `WatermarkStyle.tileOffset`，预览自然跟着重绘。
...
拖动期间显示十字参考线：一条水平线、一条垂直线，穿过水印图层的中心，`onPanEnd` 时隐藏。单块模式取文字实际绘制位置（含边界夹取），平铺模式取网格中心（画布中心加网格的整体偏移）。参考线只画在预览层、且只在照片范围内（超出部分被裁剪），不会被保存进成品图。
...
图标是「证件 + 水印」的微拟物风格：蓝色渐变底 + 一张带柔和投影的白色证件卡，卡面上平铺着斜向的淡蓝水印文字条，左上角一块浅蓝的证件照占位。设计稿是 1024×1024 的 SVG，用 Edge headless 渲染成 PNG 后缩成各尺寸。

落盘位置与尺寸：

- Android：`android/app/src/main/res/mipmap-{mdpi,hdpi,xhdpi,xxhdpi,xxxhdpi}/ic_launcher.png`，对应 48 / 72 / 96 / 144 / 192
- iOS：`ios/Runner/Assets.xcassets/AppIcon.appiconset/`，共 15 个尺寸（20/40/60、29/58/87、40/80/120、120/180、76/152、167、1024）

两点约束：**不带圆角**（各家启动器会自己加遮罩，所以四角是背景蓝色）；**不带 alpha 通道**（iOS 上架要求，存为不透明 RGB）。内容控制在中心安全区内，避免被圆形遮罩裁掉。
...
| 被测 | 要点 |
|---|---|
| `WatermarkLayout` 平铺 | 注入假测量器。断言：指令条数 > 0；所有指令 rotation 等于 −30°；字号等于短边 × 比例；超宽图与超窄图不产生空区间；空文案返回空列表；平铺偏移 (0, 0) 时结果与未偏移一致、非零偏移把整个网格平移且指令条数不变、偏移足够大时画布一侧露出完全没有水印的区域 |
| `WatermarkLayout` 单块 | 位置 (0.5, 0.5) 时居中；位置 (0, 0) 与 (1, 1) 时文字仍完整落在画布内（即夹取生效）；文字宽高超过画布时该方向居中；同一归一化位置在两种画布尺寸下产生等比的结果 |
| `WatermarkStyle` | JSON 往返序列化（含 `tileOffset`）；`singlePosition` 越界时被夹到 0–1；`tileOffset` 越界时被夹到 ±0.25；非法值（透明度越界、比例越界）被夹到合法区间 |
