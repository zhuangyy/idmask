...
页面只有主编辑页一个（选图、输文案、预览、摆位置、保存以及全部样式调整都在这一页），没有独立设置页。最近照片与最近文案都是底部弹层，两个入口各占一半宽度、并成一行，位于预览区下方、水印文字输入区之上，分别以「最近照片（N）」「最近文案（N）」唤起。这是一个工具型 App，不套 fitutor 那样的多 Tab 结构。
