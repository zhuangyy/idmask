...
**Spec:** `docs/superpowers/specs/2026-09-12-idwm-design.md`

## Global Constraints

- **Flutter 3.47.2 stable**，Dart SDK 由 `flutter create` 生成的 `environment.sdk` 约束决定，不手工降级。
- **项目位置**：Flutter 工程直接位于仓库根 `/Users/zhuangyy/ZYY/Code/idwm`，不是子目录。
- **命名（逐字照抄）**：Dart 包名 `idwm`；Android `applicationId` 与 `namespace` 均为 `com.xzgg.idwm`；iOS `PRODUCT_BUNDLE_IDENTIFIER` 为 `com.xzgg.idwm`；应用显示名（Android `android:label`、iOS `CFBundleDisplayName`）为 `证件水印`。
...
**Interfaces:**
- Consumes: 无
- Produces: 一个能跑的 Flutter 工程，包名 `com.xzgg.idwm`，4 个依赖就位，后续所有任务都在这上面写代码
...
```bash
cd /Users/zhuangyy/ZYY/Code/idwm
flutter create --org com.xzgg --project-name idwm --platforms=android,ios .
```
...
预期：Android 两处都是 `com.xzgg.idwm`；iOS 的 Runner target 是 `com.xzgg.idwm`。

若不是，手工把 `android/app/build.gradle.kts` 的 `namespace` 与 `applicationId`、以及 `ios/Runner.xcodeproj/project.pbxproj` 里 Runner target 的 `PRODUCT_BUNDLE_IDENTIFIER` 改成 `com.xzgg.idwm`（`RunnerTests` 的那几行不用动）。
...
```bash
cd /Users/zhuangyy/ZYY/Code/idwm && flutter analyze
```
...
```bash
cd /Users/zhuangyy/ZYY/Code/idwm && flutter test test/models/watermark_style_test.dart
```

预期：FAIL，报 `Target of URI doesn't exist: 'package:idwm/models/watermark_style.dart'`。
...
```bash
cd /Users/zhuangyy/ZYY/Code/idwm && flutter test test/services/template_composer_test.dart
```
...
```bash
cd /Users/zhuangyy/ZYY/Code/idwm && flutter test test/services/watermark_layout_test.dart
```

预期：FAIL，报 `Target of URI doesn't exist: 'package:idwm/services/watermark_layout.dart'`。
...
```bash
cd /Users/zhuangyy/ZYY/Code/idwm && flutter test test/services/photo_fingerprint_test.dart
```
...
```bash
cd /Users/zhuangyy/ZYY/Code/idwm && flutter test test/models/recent_photo_test.dart
```
...
```bash
cd /Users/zhuangyy/ZYY/Code/idwm && flutter test test/services/recent_photos_store_test.dart
```
...
```bash
cd /Users/zhuangyy/ZYY/Code/idwm && flutter test test/models/recent_photo_test.dart test/services/recent_photos_store_test.dart
```
...
```bash
cd /Users/zhuangyy/ZYY/Code/idwm && flutter test test/services/recent_texts_store_test.dart
```
...
```bash
cd /Users/zhuangyy/ZYY/Code/idwm && flutter test
```
...
```bash
cd /Users/zhuangyy/ZYY/Code/idwm && flutter run
```
...
- Modify: `android/app/src/main/kotlin/com/xzgg/idwm/MainActivity.kt`
...
```bash
cd /Users/zhuangyy/ZYY/Code/idwm && find android -name "MainActivity.kt"
```

预期：`android/app/src/main/kotlin/com/xzgg/idwm/MainActivity.kt`。若不是这个路径，以实际路径为准，但包名必须是 `com.xzgg.idwm`。
...
```bash
cd /Users/zhuangyy/ZYY/Code/idwm
git add lib/services/jpeg_encoder.dart android/app/src/main/kotlin/com/xzgg/idwm/MainActivity.kt ios/Runner/AppDelegate.swift
...
**Interfaces:**
- Consumes: 无
- Produces: `class BackupExcluder`，静态方法 `Future<void> exclude(String path)`；iOS 上把 `idwm_photos/` 排除出 iCloud 备份，其它平台空操作
...
class BackupExcluder {
  ...
  static const MethodChannel _channel = MethodChannel('com.xzgg.idwm/jpeg');
...
选一张照片让副本落盘，然后用 Xcode 的 Devices and Simulators 打开 App 容器，查看 `Library/Application Support/idwm_photos/` 是否带上了 `com.apple.MobileBackup` 排除属性；或观察「设置 → Apple ID → iCloud → 管理账户存储 → 备份」里本 App 的备份体积是否随副本增长。
...
```bash
cd /Users/zhuangyy/ZYY/Code/idwm && flutter analyze && flutter test
```
